    ...
    return ChatPromptTemplate.from_messages(
        [
            (
                ...
                "- WITH HISTORY: If shared conversation history or summary is available, the message is part of an ongoing session.\n"
    ...
    return ChatPromptTemplate.from_messages(
        [
            (
                ...
                "\n"
                "Context rule:\n"
                "- If the shared conversation context indicates an active reservation workflow (collecting fields, confirming details),\n"
    ...
    return ChatPromptTemplate.from_messages(
        [
            (
                ...
                "Treat CONTEXT as shared cross-graph history and use both user and assistant turns for continuity.\n"
                "Extract all fields the user has provided across the entire context, not just the last message.\n"
                "If a field is not present in the conversation and not in existing reservation data, return null for it.\n"
                "If existing reservation data contains a field and the user does not mention or contradict it, return that value (preserve it).\n"
                "Field rules:\n"
                ...
                "If the user refers by pronoun (e.g. 'this one', 'that one') and the preceding assistant message lists a facility, resolve to that facility name. "
    ...
    return ChatPromptTemplate.from_messages(
        [
            (
                ...
                "Ask for ALL missing details needed to complete the parking reservation in ONE short sentence.\n"
                "\n"
                "Rules:\n"
                "- The MISSING FIELDS block lists what the user must provide (one item per line with emoji).\n"
                "- Format your request like the example below: short intro, then list required items with emojis, one per line. But you can rephrase.\n"
                "- Do not ask for fields one by one. Combine them into one concise request.\n"
                "- Do not specify formats (e.g. YYYY-MM-DD, HH:MM) when initially asking for missing fields, unless the user asks.\n"
                "- If the VALIDATION ERRORS block is not empty, append a short ending that explains why each invalid field must be re-provided.\n"
                "- When explaining validation errors, you MUST include the specific reason from VALIDATION ERRORS (e.g. expected format, allowed range) so the user knows exactly what to fix. Do not give a generic \"invalid\" message.\n"
                "\n"
                "Example format:\n"
                "To make a reservation, I'll need a few details:\n"
                "\n"
                "📍 Facility\n"
                "📅 Date\n"
                "🕐 Start time & duration\n"
                ...
                "\n"
                "Feel free to send it all in one message!\n"
                "\n"
                "Language rule (strict):\n"
                "- Determine the user language from user turns in the CONTEXT block.\n"
                "- CONTEXT may contain both user and assistant turns from shared conversation history.\n"
                "- If the last message is short/ambiguous, infer language from earlier user turns in CONTEXT.\n"
                ...
                "- Do not translate facility names or addresses. Leave them in English.\n",
            ),
            (
                "human",
                "<<<MISSING_FIELDS>>>\n"
    ...
    return ChatPromptTemplate.from_messages(
        [
            (
                ...
                "You MUST include all reservation fields exactly once. "
            ...
            (
                "human",
                "Recent conversation:\n{conversation}\n\n"
                "Reservation details (JSON): {reservation_state}\n\n"
                "Use values only from this JSON and include exactly these fields once: "
                "{reservation_field_names_spec}.",
            ),
        ]
    )
    ...
    return ChatPromptTemplate.from_messages(
        [
            (
                ...
                "Use the shared conversation context to resolve short acknowledgements and ellipsis replies.\n"
                "Output must match the ReservationConfirmationDecision schema.\n"
                "Set confirm=true only when the user clearly agrees to submit now.\n"
                "Set confirm=false only when the user clearly refuses or cancels submission.\n"
                "Set confirm=null when the user asks to change reservation details, "
                "asks follow-up questions, hesitates, or is ambiguous.\n"
                "reasoning must be one short sentence.",
            ),
            (
                "human",
...
def reservation_already_pending_prompt() -> ChatPromptTemplate:
    return ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "Write a short user-facing response for a parking assistant when the user "
                "tries to start or continue a reservation while another reservation is still pending administrator review.\n"
                "Requirements:\n"
                "- Clearly state that another reservation cannot be started right now.\n"
                "- Clearly state that the existing reservation is still pending review.\n"
    ...
    return ChatPromptTemplate.from_messages(
        [
            (
                ...
                "- If decision is rejected, clearly say the request is rejected. Don't specify the reason.\n"
                "- Keep it concise (1-2 sentences).\n"
                "- Use the same language as the user's conversation.",
            ),
            (
